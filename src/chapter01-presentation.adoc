...
L’idée de base a pour ambition de créer un société de type _LegalTech_ footnote:[Une LegalTech peut se définir comme une startup de droit en ligne qui propose aux entreprises et aux particuliers une offre numérique]. Il s’agit donc d’un produit destiné au marché du droit.
