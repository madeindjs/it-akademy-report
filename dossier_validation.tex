...
\chapter{Présentation du projet}
  ...
  \section{Business plan}
    ...
    \begin{itemize}
      ...
      \item proposer un système de \textquote{credit} afin que les huissier pré-paye un nombre d'affaire qu'on va leur apporter
    \end{itemize}
...
\chapter{Planifier la réalisation du projet}
  ...
  \section{Méthode Kanban}
    ...
    \subsection{Utilisation de Trello}
      ...
      Dans le monde du Kanban, un logiciel SAAS se démarque parmi les autre: \href{http://trello.com/}{Trello}. Trello est un outil SAAS qui permet pousse la méthode Kanban plus loin. Cette application pousse la collaboration en ajoutant les discussions sur les tâches.
  ...
  \section{Travailler à plusieurs}\label{sec:git}
    ...
    Git\footnote{Git est un logiciel développé par Linux Torvals (fondateur de Linux) qui permet de versionner un projet. Ainsi il rend la collaboration beaucoup plus facile.} est énormément utilisé dans le monde du développement de logiciel. Son éfficacité n'est plus à prouver. Pour construire iSignif, j'ai immédiatement décidé d'appliquer la méthodologie \textbf{Git Flow}.
    ...
    \begin{description}
      ...
      \item[en bleu] cela correspond à une \verb|feature|, c'est à dire une fonctionnalité développé indépendament de l'application.
    ...
    Cette méthodologie permet ainsi de travailler à plusieurs sans se géner puisque chaque développeur peut travailler indépendament sur une branche \verb|feature|. De plus, ceci me permet de faire des mise en production régulièrement (j'en parlerai plus en détails dans la section \ref{sec:deployments}).
...
\chapter{Sécurité}
  ...
  \section{Sécutité de l'application}
    ...
    \subsection{Les failles les plus courantes}
      ...
      \paragraph{Injection SQL}
        ...
        L'injection SQL consite à injecter du code SQL\footnote{Le code SQL est utilisé pour envoyer des requêtes sur la base de données}. Comme le montre la figure \ref{fig:sql_injection}, les injection SQL sont très faciles.
  ...
  \section{Audit de sécurité}
    ...
    \subsection{Réalisation d'un scan de vulnérabilité}
      ...
      Afin de connaitre les vulnérabilité de mon instalation, j'ai décidé de faire un scan de vulnérabilité en utilisant \href{https://www.metasploit.com/}{Metasploit}\footnote{Metasploit Framework est un logiciel écrit en Ruby permettant le développement et l’utilisation d'exploit. Les exploits sont des vulnérabilités qui permettent d’exécuter du code sur une machine distante.} et \href{http://www.openvas.org/}{OpenVAS}\footnote{OpenVAS est un scanneur de vulnérabilités libre issu du fork de Nessus.}.

      Le scan de vulnérabilité est illégal à moins que le serveur nous appartienne ou bien qu'une autorisation du propriétatire est donné. Dans mons cas, le serveur m'apaprtiens.
      ...
      Plusieurs types de scan sont possibles, j'ai choisis d'utiliser le plus complet, qui est ausi le plus long. J'ai donc obtenu le résultat que l'on peu voir figure \ref{fig:openvas_report} (le rapport complet est disponnible annexe \ref{openvas_report}).
      ...
      On peut voir que globalement mon serveur possède peu de vulnérabilités. Ceci est sement du au fait que je met à jours les paquets quotidiennement et que donc, les logiciels sont à jours.
  ...
  \section{RGPD}
    ...
    \subsection{Oubliez-moi}
      ...
      \subsubsection{Exemple avec un huissier de justice}
        ...
        C'est pour cela que j'ai préféré l'\textbf{anonymization} des données.

        Etant un fan de la TDD (\textit{Test Driven Development})\footnote{Le TDD (Développement Dirigé par les Test dans la langue de molière) est une méthodologie qui consiste à développer le test unitaire avant le développement de la fonctionnalité.}, j'ai donc commencé par construire le test.
        ...
        ActiveRecord (l'ORM\footnote{Un ORM fait interface entre le code et la base de données, voir \href{https://fr.wikipedia.org/wiki/Mapping_objet-relationnel}{la définition complète sur Wikipedia}} de Ruby on Rails). J'ai donc simplement surchargé la méthode \verb|destroy| et utilisé la librairie \verb|Faker|\footnote{Il s'agit d'une librairie utilisée pour générer des fausse données en tout genre}:
